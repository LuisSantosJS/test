...
function removeDialCode(phoneNumber: string) {
  let COUNTRIES0 = COUNTRIES.map(e=> e.dial_code)
  ...
  for (let i = 0; i < COUNTRIES0.length; i++) {
    ...
    if (phoneNumber.startsWith(COUNTRIES0[i])) {
      // Remove the dial code from the phone number
      const numberWithoutDialCode = phoneNumber.slice(COUNTRIES0[i].length);
      // Return an object with the dial code and the separated phone number
      return {
        dialCode: COUNTRIES0[i] ?? 'unknown',
        number: numberWithoutDialCode ?? 'unknown',
        code: COUNTRIES.find(e=> e.dial_code == COUNTRIES0[i])?.code ?? 'unknown',
      };
    }
  }
...
export { COUNTRIES, removeDialCode };